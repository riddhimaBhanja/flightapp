...
import com.flightapp.auth.config.PasswordExpiryProperties;
...
public class AuthServiceImpl implements AuthService {
    ...
    public Mono<AuthResponse> login(AuthRequest request) {
        ...
        return userRepository.findByUsername(request.getUsername())
                ...
                .flatMap(user -> {
                    ...
                    boolean isPasswordExpired = checkIfPasswordExpired(lastChanged);
                    boolean forceChange = Boolean.TRUE.equals(user.getForcePasswordChange());
                    boolean needsPasswordChange = isPasswordExpired || forceChange;
                    ...
                    if (needsPasswordChange) {
                        if (passwordExpiryProperties.isDemoMode()) {
                            message = "Your password has expired (demo mode: "
                                    + passwordExpiryProperties.getDemoSeconds()
                                    + " seconds). You must change it to continue.";
                        } else {
                    ...
                    AuthResponse response = AuthResponse.builder()
                            ...
                            .role(user.getRole())
                            .message(message)
                            .passwordExpired(needsPasswordChange)
    ...
    private boolean checkIfPasswordExpired(LocalDateTime lastPasswordChanged) {
        if (lastPasswordChanged == null) {
            return true;
        }

        if (passwordExpiryProperties.isDemoMode()) {
            long secondsSinceLastChange = ChronoUnit.SECONDS.between(lastPasswordChanged, LocalDateTime.now());
            return secondsSinceLastChange >= passwordExpiryProperties.getDemoSeconds();
        } else {
            long daysSinceLastChange = ChronoUnit.DAYS.between(lastPasswordChanged, LocalDateTime.now());
            return daysSinceLastChange >= passwordExpiryProperties.getProductionDays();
        }
    }
                    ...
                    return passwordHistoryRepository.findByUserIdOrderByChangedAtDesc(user.getId())
                            ...
                            .flatMap(passwordHistories -> {
                                ...
                                boolean passwordReused = passwordHistories.stream()
                                        .anyMatch(ph -> passwordEncoder.matches(request.getNewPassword(),
                                                ph.getPasswordHash()));

                                if (passwordReused) {
                                    log.warn("Password change failed - password reused for user: {}",
                                            request.getUsername());
                                    return Mono.just(ChangePasswordResponse.builder()
                                            .success(false)
                                            .message(String.format(
                                                    "Password cannot be the same as your last %d passwords",
                                ...
                                return passwordHistoryRepository.save(passwordHistory)
                                        ...
                                        .onErrorResume(error -> {
                                            log.error("Error updating password for user: {}", user.getUsername(),
                                                    error);
                                            return Mono.just(ChangePasswordResponse.builder()
                                                    .success(false)
        ...
        return userRepository.findByUsername(username)
                .map(user -> {
                    ...
                    if (forceChange) {
                        ...
                        message = String.format("Your password expired %d days ago. Please change it immediately.",
                                Math.abs(daysUntilExpiry));
                    } else if (daysUntilExpiry <= 7) {
                        message = String.format("Your password will expire in %d days. Please change it soon.",
                                daysUntilExpiry);
                    } else {
