...
<div *ngIf="isForced" style="max-width: 800px; margin: 0 auto 2rem; background: linear-gradient(135deg, #FEF3C7 0%, #FDE68A
  100%); border: 3px solid #F59E0B; border-radius: 16px; padding: 2rem; text-align: center;">
    <h3 style="color: #92400E; margin: 0 0 0.5rem; font-size: 1.3rem;">⚠️ Password Expired - Action Required</h3>
    <p style="color: #78350F; margin: 0;">For security reasons, your password has expired and must be changed to continue.</p>
  </div>
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input
                ...
                placeholder="Enter new password (min 8 characters, mixed case, number, special char)"
            ...
            <span class="error-message" *ngIf="isFieldInvalid('newPassword')">
              ...
              <span *ngIf="changePasswordForm.get('newPassword')?.errors?.['pattern']">
...
export class ChangePasswordComponent {
  ...
  constructor(
   ...
   ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      if (this.isForced) {
        this.forcedUsername = localStorage.getItem('expiredPasswordUsername') || '';
      }
    });
  ...
  onSubmit() {
    ...
    // Use forcedUsername if available, otherwise use currentUser
    let username: string;
    if (this.isForced && this.forcedUsername) {
      username = this.forcedUsername;
      console.log('Using forced username:', username);
    } else {
      ...
      console.log('Using current user username:', username);
    }

    console.log('Calling changePassword API for user:', username);

    this.authService.changePassword(username, currentPassword, newPassword, confirmPassword)
      .pipe(
        ...
        catchError((error) => {
          console.error('Change password error caught in catchError:', error);
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error('Change password error in subscribe:', error);
          this.errorMessage = error.error?.message || error.message || 'Failed to change password. Please check your current password and try again.';
        }
      });
  }
